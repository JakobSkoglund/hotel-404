...
export async function getHotelInfo(){
  const hotels = await axios.get("http://localhost:7800/api/hotels/all");
...
export async function getHotelQuery(params: any){
  const hotels = await axios.get("http://localhost:7800/api/hotels/getHotels", {params});
  const formattedHotels = hotels.data.map((hotel: any) => {
    return {
      ...hotel.display,
      id: hotel._id,
...
export async function getHotelPage(id: string): Promise<IHotelDetails>{
  ...
  const hotel = await axios.get("http://localhost:7800/api/hotels/hotelDetails", {params});
